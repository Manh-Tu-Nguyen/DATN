    ...
    <div class="flex h-screen bg-gray-100">
        <aside class="w-64 bg-gray-800 text-white flex flex-col transition-all duration-300">
            <div class="p-4 text-2xl font-bold border-b border-gray-700 flex items-center gap-2">
                <Icon icon="ri:admin-fill" class="text-pink-500" /> HighSports
            </div>

            <nav class="flex-1 p-4 space-y-1 overflow-y-auto">

                <router-link to="/admin/dashboard"
                    class="flex items-center gap-3 py-2.5 px-4 rounded transition-colors hover:bg-gray-700"
                    active-class="bg-pink-600 hover:bg-pink-700">
                ...
                <div>
                    <button @click="toggleProduct"
                        ...
                        :class="{ 'bg-gray-700': isProductOpen }">
                        <div class="flex items-center gap-3">
                            <Icon icon="icon-park-solid:sport" width="20" /> Quản lý sản phẩm
                        </div>
                        <Icon icon="mdi:chevron-down" width="20" class="transition-transform duration-200"
                            :class="{ 'rotate-180': isProductOpen }" />
                    </button>

                    <div v-show="isProductOpen"
                        class="mt-1 space-y-1 bg-gray-900 rounded-lg overflow-hidden transition-all">
                        
                        <router-link to="/admin/products"
                            class="flex items-center gap-3 py-2 pl-12 pr-4 text-sm text-gray-400 hover:text-white hover:bg-gray-700 block"
                            active-class="text-pink-400 font-bold">
                            <Icon icon="mdi:shoe-sneaker" width="16" /> Sản phẩm
                        </router-link>

                        <router-link to="/admin/attributes"
                            class="flex items-center gap-3 py-2 pl-12 pr-4 text-sm text-gray-400 hover:text-white hover:bg-gray-700 block"
                            active-class="text-pink-400 font-bold">
                            <Icon icon="mdi:format-list-bulleted-type" width="16" /> Thuộc tính
                        </router-link>
                    </div>
                </div>
        ...
        <main class="flex-1 h-full overflow-y-auto">
            <header class="bg-white shadow-sm py-3 px-6 flex justify-end items-center sticky top-0 z-10">
                <div class="flex items-center gap-2">
                    ...
                    <div class="w-8 h-8 rounded-full bg-pink-500 text-white flex items-center justify-center font-bold">
                        A
                    </div>
                </div>
            </header>
